...
sys.path.append("/data/NEURO/python/uav/drone_ml_wind_estimation/")
...
def train(multi_step_model, trainData, trainLabels, testData, testLabels, Title): 
    BATCH = 16
    t = multi_step_model.fit(trainData, trainLabels, epochs=epochs, batch_size=BATCH, validation_data=(testData, testLabels))
    ...
    plt.close()
## save model
    current_date = datetime.now().strftime("%B_%d_%H%M")

    keras_model_path = f'/data/NEURO/python/uav/drone_ml_wind_estimation/wind_{current_date}_batch_{BATCH}'
    multi_step_model.save(keras_model_path, save_format="h5")
